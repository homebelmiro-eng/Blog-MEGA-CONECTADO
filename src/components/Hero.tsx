...
import { heroArticle as staticHero, heroSideArticles as staticSides } from '../data';
...
export default function Hero() {
  ...
  const [sides, setSides] = useState<Article[]>(staticSides);
    ...
    async function fetchDynamicArticles() {
      ...
      try {
        ...
        if (all && all.length > 0) {
          setHero(all[0]);
          if (all.length > 1) {
            setSides(all.slice(1, 4));
          }
        }
      } catch (error) {
        console.error('Error fetching hero articles from Firestore:', error);
      }
    }
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-20">
      <Link to={getArticleUrl(hero)} className="lg:col-span-8 group block">
        <article>
